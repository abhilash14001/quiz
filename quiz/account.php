...
<?php error_reporting($level =null);
$ppp = "select * from user_registered_quiz where userid = '$_SESSION[userid]' && paid='success'";	
$kk   = mysqli_query($con, $ppp);
$c= 0;
while($lll = mysqli_fetch_assoc($kk))
{
	$paid = $lll['paid'];
	$title = $lll['title'];
	$total = $lll['total'];
	$desc = $lll['description'];
	$correct =$lll['correct']; 
	$wrong =$lll['wrong']; 
	$time  =$lll['time']; 
	$prize  =$lll['prize']; 
...
$yavan = '<tr><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $title . '</td><td style="vertical-align:middle">' . $total . '</td><td style="vertical-align:middle">' . $desc. '</td><td style="vertical-align:middle">+' . $correct . '</td><td style="vertical-align:middle">-' . $wrong . '</td><td style="vertical-align:middle">' . $correct * $total . '</td><td style="vertical-align:middle">' . $time . '&nbsp;min</td>
...
$(function(){
	$("#given_date").countdowntimer({
       starttime : <?php error_reporting($level =null); echo date('d-m-yy H:i:sa'); ?>,
...
<div class="custom"><b><h5><i class = "fa fa-google-wallet"></i> Wallet Balance : <?php error_reporting($level =null); echo $wallet; ?>
	<br><br><i class = "fa fa-money"></i>Earned Money &nbsp;: <?php error_reporting($level =null); echo $earned; ?>
	</h5></b></div>
...
if (@$_GET['q'] == 5)
echo 'class="active"';
?>><a href="account.php?q=5"><span class="fa fa-google-wallet" aria-hidden="true"></span>&nbsp;Wallet</a></li></ul>
...
<div class="row">
<div class="col-md-12">
<?php error_reporting($level =null); error_reporting($level= null);
...
$c = 1;
$qq = "SELECT * FROM quiz WHERE status = 'enabled' ORDER BY date DESC";
...
if($paid == 'success' && $starttime >= $current_time)
{ 
echo $yavan;
}
elseif($paid!= 'success' && $starttime >= $current_time)
{
...
if (@$_GET['q'] == 4)
{
$m = "select * from user_registered_quiz where userid = '$userid' && paid = 'success'";
...
if($current_timee>= $start_time )
{
 echo '<a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="color:#FFFFFF;background:darkgreen;font-size:12px;padding:7px;padding-left:10px;padding-right:10px"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span><b>Start</b></span></a>'; 
}
else
{
echo '<button class="btn btn-danger">Still time for quiz not started</button>';
}echo '
</b></td>
...
$prize=$row['prize'];
$eid = $row['eid']; */
$iip = "select * from user where id ='$_SESSION[userid]'";
...
<td style="vertical-align:middle"><b>Wallet Balance</b></td>
<td style="vertical-align:middle"><b>Earned Money</b></td>
<td style="vertical-align:middle"><b>Withdraw Earned Money</b></td>
<tr>
<td style="vertical-align:middle">' . $i . '</td>
<td style="vertical-align:middle">' . $title . '</td>
<td style="vertical-align:middle">' . $wall . '</td>
<td style="vertical-align:middle">'.$earned.'</td>
';

if($earned > 20.00)
{
echo '<td style="vertical-align:middle"><button type="submit" class="btn btn-success">Withdraw Money</button></td>';
}
else
{
echo '<td style="vertical-align:middle"><b><font color="red"> Minimum withdraw balance should be greater than Rs. 20</font></b></td>';
